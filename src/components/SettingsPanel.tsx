...
const SettingsPanel = forwardRef<HTMLDivElement, SettingsPanelProps>(
  ({ isOpen, onClose }, ref) => {
    ...
    const tabs = [
      { id: 'ai-providers' as const, icon: Bot, label: 'AI' },
    ...
    const getTestStatusIcon = (integrationId: string) => {
      ...
      switch (status) {
        ...
        case 'success':
          return <CheckCircle className="h-3 w-3 sm:h-4 sm:w-4 text-neon-green" />;
        case 'error':
    ...
    return (
      <div ref={ref} className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-background/80 backdrop-blur-sm animate-fade-in">
        <div className="w-full max-w-3xl glass-panel max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col animate-scale-in">
          {/* Header */}
          <div className="flex items-center justify-between p-3 sm:p-4 border-b border-border/60">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center">
                <Settings className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />
              </div>
              <div>
                <h2 className="text-base sm:text-lg font-display font-semibold text-foreground">Settings</h2>
                <p className="text-xs sm:text-sm text-muted-foreground hidden sm:block">Manage your AI providers and preferences</p>
              </div>
            </div>
          ...
          <div className="flex flex-1 overflow-hidden flex-col sm:flex-row">
            {/* Sidebar */}
            <div className="sm:w-36 md:w-48 border-b sm:border-b-0 sm:border-r border-border/60 p-1.5 sm:p-2 bg-card/30">
                ...
                {tabs.map(tab => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm transition-all duration-200 whitespace-nowrap flex-shrink-0 sm:w-full",
                        activeTab === tab.id
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-4 sm:space-y-6 scrollbar-thin">
              {activeTab === 'ai-providers' && (
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    <div>
                      <h3 className="text-xs sm:text-sm font-medium mb-1">Default AI Provider</h3>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2">
                      {aiProviders.map(provider => (
                        <button
                          ...
                          className={cn(
                            "flex items-center gap-1.5 sm:gap-2 p-2 sm:p-3 rounded-lg sm:rounded-xl text-left transition-all duration-200",
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-xs sm:text-sm font-medium truncate">{provider.name}</div>
                            {selectedProvider === provider.id && (
                              <Check className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-primary inline" />
                            )}
                          </div>
                        </button>
                      ))}
                  ...
                  {currentProvider && (
                    <div className="space-y-2 sm:space-y-3">
                      <div>
                        <h3 className="text-xs sm:text-sm font-medium mb-1">Default Model</h3>
                        <p className="text-xs text-muted-foreground">
                          Choose the AI model to use by default
                        </p>
                      </div>
                      <Select value={selectedModel} onValueChange={setSelectedModel}>
                        <SelectTrigger className="bg-secondary/50 border-border h-9 sm:h-10 text-xs sm:text-sm">
                          ...
                          {currentProvider.models.map(model => (
                            <SelectItem key={model.id} value={model.id} className="text-xs sm:text-sm">
                              <div className="flex items-center gap-1.5 sm:gap-2">
                                ...
                                <span className="text-xs text-muted-foreground hidden sm:inline">- {model.description}</span>
                              </div>
                            </SelectItem>
                          ))}
                        </SelectContent>
                  ...
                  <div className="space-y-3 sm:space-y-4 pt-3 sm:pt-4 border-t border-border/60">
                    <div>
                      <h3 className="text-xs sm:text-sm font-medium mb-1">API Keys</h3>
                      <p className="text-xs text-muted-foreground">
                        Add your API keys to use your own accounts.
                      </p>
                    </div>

                    <div className="space-y-3 sm:space-y-4">
                      {aiProviders.filter(p => p.id !== 'lovable').map(provider => (
                        <div key={provider.id} className="space-y-1.5 sm:space-y-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1.5 sm:gap-2">
                              <span className="text-base sm:text-lg">{provider.icon}</span>
                              <label className="text-xs sm:text-sm font-medium">{provider.name}</label>
                            </div>
                            <a 
                            ...
                            >
                              Docs <ExternalLink className="h-2.5 w-2.5 sm:h-3 sm:w-3" />
                            </a>
                          </div>
                          <Input
                            type="password"
                            ...
                            className="bg-secondary/50 border-border font-mono text-xs sm:text-sm h-8 sm:h-10"
                          />
                          {apiKeys[provider.id] && (
              ...
              {activeTab === 'profile' && (
                <div className="space-y-3 sm:space-y-4">
                  <div>
                    <h3 className="text-xs sm:text-sm font-medium mb-2 sm:mb-3">Profile Information</h3>
                    <div className="space-y-3 sm:space-y-4">
                      <div className="space-y-1.5 sm:space-y-2">
                        <label className="text-xs sm:text-sm text-muted-foreground">Email</label>
                        <Input value={user?.email || ''} disabled className="bg-secondary/50 h-8 sm:h-10 text-xs sm:text-sm" />
                      </div>
                      <div className="space-y-1.5 sm:space-y-2">
                        ...
                        <Input
                          ...
                          className="bg-secondary/50 h-8 sm:h-10 text-xs sm:text-sm"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 sm:pt-4 border-t border-border/60">
                    <h3 className="text-xs sm:text-sm font-medium mb-2 sm:mb-3 text-destructive">Danger Zone</h3>
                    <Button variant="destructive" onClick={signOut} className="gap-1.5 sm:gap-2 text-xs sm:text-sm h-8 sm:h-10">
              ...
              {activeTab === 'integrations' && (
                <div className="space-y-4 sm:space-y-6">
                  <div>
                    <h3 className="text-xs sm:text-sm font-medium mb-1">External Services</h3>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2 sm:mb-3">
                            <div className="flex items-center gap-2 sm:gap-3">
                              <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-md sm:rounded-lg bg-secondary/50 flex items-center justify-center">
                                {integration.icon}
                              </div>
                              <div>
                                <div className="flex items-center gap-1.5 sm:gap-2">
                                  <span className="text-xs sm:text-sm font-medium">{integration.label}</span>
                                  {hasAnyValue && (
                                    <span className="flex items-center gap-0.5 sm:gap-1 text-xs text-neon-green">
                                      <Check className="h-2.5 w-2.5 sm:h-3 sm:w-3" />
                                      Active
                                    </span>
                                  )}
                                </div>
                                <p className="text-xs text-muted-foreground hidden sm:block">{integration.description}</p>
                              </div>
                            </div>
                            <a 
                              ...
                              target="_blank" 
                              rel="noopener noreferrer"
                              className="text-xs text-primary hover:underline flex items-center gap-0.5 sm:gap-1"
                            >
                              <span className="hidden sm:inline">Get Key</span> <ExternalLink className="h-2.5 w-2.5 sm:h-3 sm:w-3" />
                            </a>
                          </div>
                            ...
                            {integration.fields?.map(field => (
                              <div key={field.id} className="space-y-1">
                                ...
                                <Input
                                  ...
                                  className="bg-background/50 border-border font-mono text-xs sm:text-sm h-8 sm:h-9"
                                />
                              </div>
                            ))}
                          </div>
                          ...
                          {integration.testable && hasAnyValue && (
                            <div className="mt-3 sm:mt-4 pt-2 sm:pt-3 border-t border-border/40">
                              <Button
                                ...
                                className="gap-1.5 sm:gap-2 text-xs h-7 sm:h-8"
                              >
                  ...
                  <div className="p-3 sm:p-4 rounded-lg sm:rounded-xl bg-accent/10 border border-accent/20">
                    <div className="flex items-start gap-2 sm:gap-3">
                      <Shield className="h-4 w-4 sm:h-5 sm:w-5 text-accent mt-0.5" />
                      <div>
                        <h4 className="text-xs sm:text-sm font-medium text-accent">Security Note</h4>
              ...
              {activeTab === 'theme' && (
                <div className="space-y-3 sm:space-y-4">
                  <div>
                    <h3 className="text-xs sm:text-sm font-medium mb-2 sm:mb-3">Theme</h3>
                      ...
                      {themeOptions.map(option => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "flex flex-col items-center gap-1.5 sm:gap-2 p-3 sm:p-4 rounded-lg sm:rounded-xl transition-all duration-200",
                              theme === option.id
                  ...
                  <div className="space-y-2 sm:space-y-3 pt-3 sm:pt-4">
                    <h3 className="text-xs sm:text-sm font-medium">Preferences</h3>
                    <div className="flex items-center justify-between py-2 px-2 sm:px-3 rounded-lg bg-secondary/30">
                      <div>
                        <div className="text-xs sm:text-sm">Animations</div>
                        <div className="text-xs text-muted-foreground hidden sm:block">Enable motion effects</div>
                      </div>
                      <Switch defaultChecked />
                    </div>
                    <div className="flex items-center justify-between py-2 px-2 sm:px-3 rounded-lg bg-secondary/30">
                      <div>
                        <div className="text-xs sm:text-sm">Sound Effects</div>
                        <div className="text-xs text-muted-foreground hidden sm:block">Play sounds on actions</div>
                      </div>
                      <Switch />
                    </div>
                  </div>
          ...
          {/* Footer */}
          <div className="flex items-center justify-end p-3 sm:p-4 border-t border-border/60 bg-card/30 gap-2">
            <Button variant="ghost" onClick={onClose} size="sm" className="text-xs sm:text-sm h-8 sm:h-10">
              Cancel
            </Button>
            <Button onClick={saveSettings} disabled={saving} className="gap-1.5 sm:gap-2 text-xs sm:text-sm h-8 sm:h-10">
